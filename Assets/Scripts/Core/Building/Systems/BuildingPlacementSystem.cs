...
public partial class BuildingPlacementSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        if (!physicsWorld.CollisionWorld.CastRay(mainRayInput, out PhRaycastHit mainHit))
            return;

        var lt = em.GetComponentData<LocalTransform>(previewEntity);
        bool isFoundationPreview = em.HasComponent<FoundationTag>(previewEntity);

        bool placementIsValid;

        if (isFoundationPreview)
        {
            placementIsValid = HandleFoundationPlacement(ref lt, previewEntity, mainRay, mainHit, in physicsWorld, in settings);
        }
        else
        {
            placementIsValid = HandleRegularBuildingPlacement(ref lt, previewEntity, mainHit.Position, in physicsWorld, in settings);
        }
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (isFoundationPreview && em.HasComponent<PlacementValidTag>(previewEntity))
            {
                float baseH = 1f;
    ...
    private bool HandleRegularBuildingPlacement(ref LocalTransform lt, Entity previewEntity, float3 centerHitPos, in PhysicsWorldSingleton physicsWorld, in BuildingSettings settings)
    {
        var em = EntityManager;
